...
function searchDocuments(req, res) {
  ...
  const targetDocumentTitle = req.query.q;
  if (targetDocumentTitle === undefined || targetDocumentTitle === '') {
    res.status(400)
      .json({
  ...
  Document
    .findAll({
      where: {
        // NB: There's $iLike too, but right now that's for Postgres alone
        // and we try to be as SQL dialect agnostic as possible. Besides,
        // this one, or at least this format, is case-insensitive too.
        title: { $like: `%${targetDocumentTitle}%` },
        $or: [
          { access: 'public' },
